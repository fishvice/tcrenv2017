...
**TODO**: Should preferable try something like a glm-model. But then would need all the data.
...
We want to end with some analysis that show the distribution of total effort, total catch and catches by some selected species, e.g. by ICES statistical rectangle. And more importantly via some finer arbitrary rectangle size.
...
* Read in two logbook datatables
    ...
    * The catch table (no subsamples here) - see our `sharepoint`: http://community.ices.dk/TrainingCourses/tcrenv/2014%20Meeting%20docs/data-raw/logbook_catch.csv
...
d <- 
  Station %>% 
  mutate(sq = gisland::encode(lon, lat, resolution = c(width, height)),
         year = year(date)) %>% 
  group_by(year, sq) %>% 
  summarize(effort = sum(towtime, na.rm = TRUE)) %>%
