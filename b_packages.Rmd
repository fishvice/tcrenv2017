...
Now we may not always have the exact code in our head each time, so lets create a function and then call the function in the code (Ignore the comments at the beginning of the script for now, it will become apparent in the next section on generating packages):
...
* For the last three arguments of the function we passed a default value but this is not really needed (and may create some confusion because these parameters are stock specific, but lets just say that these are parameters for the stock you most frequently work with).

Note that when you run the above script you will see that you have generated an object (object class is a function) **vb** in your global environment:
...
Here the function is inside a particular R-script or rmarkdown document in a particular project it. To make it accessible for other projects it is best to store the function in a specific "global" directory and then source the function.
...
In [@wickham2016r] there is a nice chapter on [Functions](http://r4ds.had.co.nz/functions.html).


## Generating an R-package
___

An R-packages is just a set of functions and data with hopefully some documentation (not absolutely necessary) that reside in a directory that has a specific structure. One can easily create a private packages that contains functions that one frequently reuses. This document snippet provides the minimum steps needed to generate a private R-package - but including the step of documenting the functions (the thing you get when doing e.g. ```?mean```). If you want to learn more about R-packages, the best current source is the book [R packages](http://r-pkgs.had.co.nz) [@wickham2015r].
