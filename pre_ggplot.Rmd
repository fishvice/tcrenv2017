...
Splitting a graph into subsets based on a categorial variable. 
...
The last plot a linear regression is fitted (check out ?lm).
...
... here some nice introductory text
...
The graph shows the number of observations we have of each maturity stage. Here the argument for stat in the `layer`function is no longer "identity" but "count":
...
For continous data one uses the 'geom_histogram'-function (left default bin-number, right bindwith specified as 50 cm): 
...
One can add another varible (left) or better use facet (right):
...
Alternatives to histograms for continuous data are frequency polyogons:
```{r, out.width = "50%", fig.show = "hold"}
...
```{r, out.width = "50%", fig.show = "hold"}
p <- ggplot(d, aes(maturity, length)) + geom_boxplot()
p <- ggplot(d, aes(reorder(maturity, length), length)) + geom_boxplot()
p
```

It is sometimes juseful to plot the "raw" data over summary plots. Using `geom_point` as an overlay of the above plot (left) is not very useful, `geom_jitter` coming to the rescue (right):
```{r, out.width = "50%", fig.show = "hold"}
...
* Raw data with median lenght at age (red) superimposed
