...
# plot wind
require(plotrix, quietly=TRUE)      # containing feather.plot
...
feather.plot2(wind$windforce, wind$winddir, fp.type="m", xlabels=wind$haul)

wind %>%
  filter(winddir <= 360) %>%
  ggplot(aes(haul, winddir)) +
  geom_point()

wind %>%
  filter(winddir <= 360) %>%
  ggplot() +
  geom_segment(aes(x = haul, xend = winddir, y = 0, yend = windforce))
