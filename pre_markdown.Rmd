...
# tab <- data_frame(x=1:3,a=letters[x])
...
R codes in RMarkdown are not limited to chunks. R commands can also be evaluated inline, that within a paragraph. This is done using the following command:

~~~
# this is paragraph with some result: `r 3*3 ` 
~~~
...
You can control many other “whole document” settings by tweaking the parameters of the YAML header.  YAML stands for “yet another markup language”, which is designed for representing hierarchical data in a way that’s easy for humans to read and write. R Markdown uses it to control many details of the output. Many of the settings in YAML can be set using the options menu in Rstudio. Notably:
