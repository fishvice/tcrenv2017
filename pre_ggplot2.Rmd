...
The above demonstrates that a spatial map in its simplest term is just an xy-plot with a specific projection. Note that the `coord_quickmap` is only an approximation, if one is operating on a fine scale `coord_map` may be more accurate.
...
2. Read the `coord_map` help file for projections other than the default "mercator". Trey to create a map that looks something along:
...
ns_area <- sf::st_read(paste0(tmpdir, "/Shapefile.dbf"), quiet = TRUE)
class(ns_area)
```
...
station  <- read_csv("data-raw/logbook_station.csv")
glimpse(station)
```
...
```{r, echo = FALSE, eval = FALSE}
catch <- read_csv("data-raw/logbook_catch.csv")
station %>% 
  mutate(year = year(date),
         sq = geo::d2ir(lat, lon)) %>% 
